...
show_help() {
  cat << EOF
使用法: $0 <リポジトリ名> <開始日> <終了日> [出力ファイル]

引数:
  リポジトリ名  - 「オーナー/リポジトリ」形式のリポジトリ名 (例: octocat/hello-world)
  開始日       - YYYY-MM-DD形式の開始日
  終了日       - YYYY-MM-DD形式の終了日
  出力ファイル  - (オプション) 結果を保存するJSONファイル

例:
  $0 octocat/hello-world 2023-01-01 2023-01-31 prs.json
EOF
  exit 1
}
...
# PRのリストを取得
PRS=$(gh pr list --repo "$REPO" --search "created:$DATE_FROM..$DATE_TO" --json number,title,url,createdAt,author,state --limit 100)
...
echo "$PRS" | jq -c '.[]' | while read -r pr; do
  ...
  FORMATTED_COMMENTS=$(echo "$PR_COMMENTS" | jq '.comments[] | {author: .author.login, body: .body, created_at: .createdAt}')
